    ...
    actions: {
        ...
        async signIn(signInRequest, router) {
            try {
                ...
                const signInResponse = new SignInResponse(response.data.id, response.data.email, response.data.token);

                if (!signInResponse.id || signInResponse.id <= 0) {
                    throw new Error("ID inválido al iniciar sesión.");
                }
                ...
                console.log("✅ SignInResponse creado:", signInResponse);

                return signInResponse;

            } catch (error) {
                console.error("❌ Error en signIn store:", error);
